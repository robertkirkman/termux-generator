...
build_packages_only() {
    local build_packages_archs=""
    if [ -n "${BOOTSTRAP_ARCHITECTURES}" ]; then
        build_packages_archs="${BOOTSTRAP_ARCHITECTURES}"
    elif [[ "$TERMUX_APP_TYPE" == "f-droid" ]]; then
        build_packages_archs="aarch64,x86_64,arm,i686"
    else
        build_packages_archs="aarch64,x86_64,arm"
    fi
    ...
    echo "[*] Both --disable-bootstrap and --add were specified. Building packages without creating a bootstrap..."

    # Make build-packages.sh executable
    chmod +x "$TERMUX_GENERATOR_HOME/build-packages.sh"

    # Execute build-packages.sh
    "$TERMUX_GENERATOR_HOME/build-packages.sh" \
        --name "${TERMUX_APP__PACKAGE_NAME}" \
        --type "${TERMUX_APP_TYPE}" \
        --architectures "${build_packages_archs}" \
        --packages "${ADDITIONAL_PACKAGES}"

    # Clean the built-packages.zip generated by build-packages.sh to avoid duplicates
    rm -f "$TERMUX_GENERATOR_HOME/built-packages.zip"

    # Move the individual .deb packages directly to the root workspace directory
    # so they are uploaded by GitHub Actions actions/upload-artifact step
    if [ -d "$TERMUX_GENERATOR_HOME/built-packages" ]; then
        for deb in "$TERMUX_GENERATOR_HOME/built-packages"/*.deb; do
            if [ -f "$deb" ]; then
                mv "$deb" "$TERMUX_GENERATOR_HOME/${TERMUX_APP__PACKAGE_NAME}-${TERMUX_APP_TYPE}-$(basename "$deb")"
                echo "[*] Moved $(basename "$deb") to root directory"
            fi
        done
        rm -rf "$TERMUX_GENERATOR_HOME/built-packages"
    fi
}
